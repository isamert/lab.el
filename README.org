...
If you work with more than one GitLab instance, use ~lab-config~ instead:

#+begin_src elisp
(setq lab-config
      '((:host "https://gitlab.com"
         :token "YOUR-PRIVATE-GITLAB-API-TOKEN"
         :group "my-group")
...
~:token~ is optional in each entry — if omitted, it is looked up via
auth-source using the ~:host~ value. ~:group~ is also optional (see
~lab-group~ for details).

When multiple hosts are configured, lab.el automatically determines
which host to use based on:
- The git remote of the current repository, or
- The URL of the merge request/pipeline being acted on.

If neither applies (e.g., listing MRs from a non-repo buffer), the
first entry in ~lab-config~ is used.
...
- Merge Requests ::
  - Conduct code reviews totally inside Emacs with
    ~lab-open-merge-request-diff~!
  - Show merge request overview and threads in an org buffer with
    ~lab-merge-request-overview~.
...
- Jobs ::
  ...
  - Act on jobs:
    - Open them in browser.
    - Run manual jobs; trigger retries, cancellation or deletion.
...
| ~C-c ; u~   | lab-approve-merge-request          | Approve the merge request.                                                                                          |
| ~C-c ; a~   | lab-unapprove-merge-request        | Unapprove the merge request.                                                                                        |
|-----------+------------------------------------+---------------------------------------------------------------------------------------------------------------------|
...
This is a similar interface to the "Overview" tab of the GitLab Merge
Request view, where you can see the merge request summary and all the
activity in chronological order, such as commits, opened threads,
replies to those threads, changes, etc.

Either use the function ~lab-open-merge-request-overview~ and give it a
merge request link or use any of the following functions to find a
merge request:
...
and then open the overview view by issuing the ~threads & overview~ action.


Like in the merge request diff buffer, you can use the following
functions to manipulate threads and the merge request itself:
...
Only difference is that these are applied immediately, you don't need
to do ~lab-send-review~. For example, to edit a thread/comment, simply
change the text and then hit ~C-c ; e~ (or do ~M-x lab-edit-thread~) and
the change will be applied immediately. Similarly, to add a reply to a
thread ~C-c ; r~ (or do ~M-x lab-reply-thread~), write your reply and then
hit ~C-c C-c~. It will be sent immediately and the overview buffer will
be updated.

The following functionality is also available, similarly to diff
buffer.
...
The list may be outdated, see ~M-x desribe-keymap
lab-merge-request-overview-prefix-map~ for an up-to-date list.
